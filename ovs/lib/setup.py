...
class SetupController(object):
    ...
    def _prepare_node(cluster_ip, nodes, known_passwords, hypervisor_info, auto_config, disk_layout):
        ...
        for node in nodes:
            node_client = SSHClient.load(node, passwords[node])
            root_authorized = node_client.file_read(authorized_keys_filename.format(root_ssh_folder), "")
            root_authorized_arr = root_authorized.split('\n')
            ovs_authorized = node_client.file_read(authorized_keys_filename.format(ovs_ssh_folder), "")
            ovs_authorized_arr = ovs_authorized.split('\n')
            for i in filter(lambda a: a != '', authorized_keys.split('\n')):
                if i not in root_authorized_arr:
                    root_authorized += '\n{0}\n'.format(i)
                if i not in ovs_authorized_arr:
                    ovs_authorized += '\n{0}\n'.format(i)
            node_client.file_write(authorized_keys_filename.format(root_ssh_folder), root_authorized)
            node_client.file_write(authorized_keys_filename.format(ovs_ssh_folder), ovs_authorized)
...
""".format(cluster)
        ...
        target_client.run('mkdir -p /opt/data/elasticsearch/tmp')
        ...
        SetupController._replace_param_in_config(target_client, '/etc/logstash/conf.d/indexer.conf',
                                                    '<CLUSTER_NAME>', 'ovses_{0}'.format(cluster_name))
        if Osdist.is_ubuntu(target_client):
            SetupController._change_service_state(target_client, 'logstash', 'restart')
        else:
            target_client.run("/etc/init.d/logstash restart")
...
""".format(ovs_config.get('core', 'broker.port'),
        ...
        if not 'ovs' in users:
            print "  ovs is not in the user lists, we will add one!"
        ...
        output = target_client.run('sleep 5;rabbitmqctl set_policy ha-all "^(volumerouter|ovs_.*)$" \'{"ha-mode":"all"}\'', quiet=True)
        if Osdist.is_ubuntu(target_client):
            output = output.split('\r\n')
        else:
            output = output.split('\n')
...
""".format(cluster_name, node_name, 'master', SetupController.avahi_filename))
    ...
    def _setup_extra_node(cluster_ip, master_ip, cluster_name, unique_id, nodes, ovs_config, hypervisor_info):
        ...
        if Osdist.is_ubuntu(client):
            ...
            client.run("/etc/init.d/logstash restart")
...
"""
        status = SetupController._exec_python(client,
                                              run_service_script.format(name)).split('\n')[-1]
        print "_get_service_status {0} {1}".format(name, status)
        if status == 'True':
            return True
        if status == 'False':
            return False
...
"""
    ...
    def _partition_disks(client, partition_layout):
        if Osdist.is_ubuntu(client):
            ...
            fstab_entry = 'LABEL={0}    {1}         ext4    defaults,nofail,noatime,discard    0    2 \n'
        ...
        for disk, partitions in partitions_by_disk.iteritems():
            ...
            for mp, percentage, label in partitions:
                if start == '2MB':
                    ...
                    client.run('parted {0} -s mkpart {1} {2}% {3}%'.format(disk, label, start, size_in_percentage))
                print 'mkfs.ext4 -q {0} -L {1}'.format(disk + str(count), label)
                client.run('umount -l {0}; echo $?; mount -l; mkfs.ext4 -q {1} -L {2}; mount -l'.format(disk + str(count), disk + str(count), label))
                print '{0} formated!'.format(disk+str(count))
        ...
        client.run('python -mplatform | grep Ubuntu && mountall -q || mount -a')
        ...
        if action is None:
            ...
            while safetycounter < timeout:
                ...
                if (status is False and state == 'stop') or (status is True and state in ['start', 'restart']):
                    print ' [{0}] {1} {2}ed! '.format(client.ip, name, state)
                    break
                safetycounter += 1
                time.sleep(1)
    ...
    def _update_es_configuration(es_client, value):
        ...
        if Osdist.is_ubuntu(es_client):
            SetupController._change_service_state(es_client, 'logstash', 'restart')
        else:
            es_client.run('/etc/init.d/logstash restart')
...
for config_file in os.listdir('/opt/OpenvStorage/config/voldrv_vpools'):
    ...
    def _is_rabbitmq_running(client, check_ovs=False):
        ...
        if 'unrecognized service' in output or 'Loaded: not-found' in output:
            output = None
        if output:
            if Osdist.is_ubuntu(client):
                output = output.split('\r\n')
        ...
        if 'stop/waiting' in output or 'inactive (dead)' in output:
            pass
        if 'start/running' in output or 'active (running)' in output:
            if 'start/running' in output:
                pid = output.split('process ')[1].strip()
            else:
                pid = output.split('Main PID: ')[1].split('(')[0].strip()
