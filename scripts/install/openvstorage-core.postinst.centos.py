...
def update_ssh_settings():
    ...
    check_output('service sshd reload', shell=True)
...
def update_ovs_config():
    replace_param_in_config('/opt/OpenvStorage/config/ovs.cfg',
                            'celery.path =',
                            'celery.path = /usr/bin/celery')
    replace_param_in_config('/opt/OpenvStorage/config/ovs.cfg',
                            'ip.path =',
                            'ip.path = /usr/sbin/ip')
...
def setup_ssh_key_authentication():
    ...
    if not os.path.exists(private_key_filename.format(root_ssh_folder)):
        check_output("echo y | ssh-keygen -t rsa -b 4096 -f {0} -N ''".format(private_key_filename.format(root_ssh_folder)), shell=True)
    # Generate keys for ovs
    check_output('su - ovs -c "mkdir -p {0}"'.format(ovs_ssh_folder), shell=True)
    check_output('su - ovs -c "echo y | ssh-keygen -t rsa -b 4096 -f {0} -N \'\'"'.format(private_key_filename.format(ovs_ssh_folder)), shell=True)
    ...
    for filename in [root_authorized_keys, root_known_hosts]:
        ...
        #check_output('cat {0}/id_rsa.pub >> {1}'.format(root_ssh_folder, root_authorized_keys), shell=True)
    ...
    for filename in [ovs_authorized_keys, ovs_known_hosts]:
        ...
        #check_output('cat {0}/id_rsa.pub >> {1}'.format(ovs_ssh_folder, ovs_authorized_keys), shell=True)
